...
async function init() {
  ...
  if (!liffId) {
    // If not found in config.js (e.g. Firebase Hosting static serving), try backend config API
    try {
      const configRes = await fetch('/api/config');
      if (configRes.ok) {
        const config = await configRes.json();
        if (config.liffId) {
          startLiff(config.liffId);
          return;
        }
      }
    } catch (e) {
      console.warn('Fallback config API failed:', e.message);
    }
    setStatus('ข้อผิดพลาด: ไม่พบ LIFF ID ใน config.js หรือ API', true);
    return;
  }
...
btnShareCoach.addEventListener('click', async () => {
  ...
  const coachCardFlex = {
    ...
    contents: {
      ...
      hero: {
        type: 'image',
        url: 'https://wutthipong.info/wp-content/uploads/2021/07/nen.jpg',
        size: 'full',
        aspectRatio: '1:1',
        aspectMode: 'cover'
      },
